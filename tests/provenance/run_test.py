...
def run_forkserver(test, guest, qemu, solver_bin, workdir):
    ...
    try:
        ...
        # Iterate children until the plan reports no remaining mods (the
        # tracer always writes remaining after the analyze-result round
        # trip).  Non-binradar forkserver tests see remaining == 0 after
        # the first iteration, i.e. exactly one child as before.
        remaining = 1
        child_status = -1
        for _ in range(20):
