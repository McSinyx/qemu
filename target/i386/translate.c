 ...
 * the provenance and OSPREY consumers with per-consumer gates.  The
 * parallel gen_prov_* / gen_osprey_* call-site lists are gone. */

static void gen_sem_reg_invalidate(int reg_idx, target_ulong pc) {
    if (!sem_events_active()) return;
    TCGv_i32 t_reg = tcg_const_i32(reg_idx);
    TCGv t_pc = tcg_const_tl(pc);
    gen_helper_sem_reg_invalidate(cpu_env, t_reg, t_pc);
...
static void gen_sem_reg_copy(int dst_idx, int src_idx, target_ulong pc) {
    ...
    TCGv t_pc = tcg_const_tl(pc);
    /* Pass register values as explicit args (env->regs may be stale
     * inside the helper). */
    gen_helper_sem_reg_copy(cpu_env, t_dst, t_src,
...
/* lea dst, [base + disp] (translation-time disp).  The exact insn PC is
...
/* lea dst, [base + index*scale + disp]: runtime delta variant.  The
 * delta (dst_val - pre_base) is computed at translation time via TCG;
 * the OSPREY consumer is a documented no-op (indexed lea has no
 * constant offset; preserves today's stale-origin behavior).  pre_base
 * is the base register's value BEFORE the instruction (required when
 * dst == base). */
static void gen_sem_reg_lea_dyn(int dst_idx, int base_idx, target_ulong pc,
                                TCGv pre_base) {
    if (!sem_events_active()) return;
    TCGv t_pc = tcg_const_tl(pc);
    gen_helper_sem_set_pc(cpu_env, t_pc);
    ...
    gen_helper_sem_reg_lea_dyn(cpu_env, t_dst, t_base, t_delta,
...
static void gen_sem_reg_addsub_imm(int reg_idx, int64_t delta, TCGv pre,
                                   target_ulong pc) {
    if (!sem_events_active()) return;
    TCGv_i32 t_reg = tcg_const_i32(reg_idx);
     ...
     * cpu_regs[reg_idx] is the post-op value. */
    gen_helper_sem_reg_addsub_imm(cpu_env, t_reg, t_delta, pre,
...
/* reg-reg ADD/SUB: fold the untagged operand's concrete value into the
 * tagged operand's offset (see provenance_addsub_reg). */
static void gen_sem_reg_addsub_reg(int dst_idx, int src_idx, int is_sub,
                                   target_ulong pc) {
    if (!sem_events_active()) return;
    TCGv_i32 t_dst = tcg_const_i32(dst_idx | (is_sub << 16));
    TCGv_i32 t_src = tcg_const_i32(src_idx);
    TCGv t_pc = tcg_const_tl(pc);
    gen_helper_sem_reg_addsub_reg(cpu_env, t_dst, t_src, t_pc,
                                  cpu_regs[dst_idx], cpu_regs[src_idx]);
    tcg_temp_free_i32(t_dst);
    tcg_temp_free_i32(t_src);
    tcg_temp_free(t_pc);
}

static void __attribute__((unused)) gen_sem_clobber_caller_saved(void) {
    if (!sem_events_active()) return;
 ...
 * (uniform rule; see helper_sem_on_store in sem-events.c).  `pc` rides
 * the pending raw transfer-PC scratch (same protocol as the two-helper
 * LEA sequence) so OSPREY can gate F03/F04 publication on the
 * main-image producer gate. */
static void gen_sem_on_store_class(int src_idx, TCGv t_addr, TCGMemOp ot,
                                   SemOpClass cls, target_ulong pc) {
    if (!sem_events_active()) return;
    /* Ride the pending raw transfer-PC scratch (same protocol as the
     * two-helper LEA sequence): the store helper consumes the
     * instruction PC so OSPREY can gate F03/F04 publication on the
     * main-image producer gate. */
    {
        TCGv t_pc = tcg_const_tl(pc);
        gen_helper_sem_set_pc(cpu_env, t_pc);
...
static void gen_sem_on_store(int src_idx, TCGv t_addr, TCGMemOp ot,
                              target_ulong pc) {
    gen_sem_on_store_class(src_idx, t_addr, ot, SEM_OP_INTEGER, pc);
}

static void gen_sem_on_store_x87(int src_idx, TCGv t_addr, TCGMemOp ot,
                                   target_ulong pc) {
    gen_sem_on_store_class(src_idx, t_addr, ot, SEM_OP_X87_HELPER, pc);
}
...
static SemProducerId sem_default_producer(SemOpClass cls)
{
    switch (cls) {
    case SEM_OP_SIMD:
        return SEM_PRODUCER_SIMD_SCALAR;
    case SEM_OP_X87_HELPER:
        return SEM_PRODUCER_X87_SCALAR;
    case SEM_OP_ATOMIC_RMW:
        return SEM_PRODUCER_ATOMIC_LOCK_RMW;
    case SEM_OP_PAIRED:
        return SEM_PRODUCER_PAIRED_CMPXCHG8B;
    case SEM_OP_INTEGER:
    default:
        return SEM_PRODUCER_INTEGER_MODRM;
    }
}
...
static void gen_sem_mem_access_flags_class_producer(
    TCGv t_addr, TCGMemOp ot, target_ulong pc, uint32_t flags,
    SemOpClass cls, SemProducerId producer) {
    if (!sem_events_active()) return;
    TCGv t_size = tcg_const_tl(1 << (ot & MO_SIZE));
    TCGv t_pc = tcg_const_tl(pc);
    TCGv_i32 t_flags = tcg_const_i32(SEM_MEM_FLAGS_WITH_POLICY(
        flags, SEM_INTERVAL_EXACT_WIDTH, producer));
    TCGv_i32 t_cls = tcg_const_i32(cls);
...
static void gen_sem_mem_access_flags_class(TCGv t_addr, TCGMemOp ot,
    ...
    gen_sem_mem_access_flags_class_producer(
        t_addr, ot, pc, flags, cls, sem_default_producer(cls));
}
...
/* Raw-size variant for non-power-of-two x87/helper intervals.  The
 * policy-only suffix is intentional: it keeps interval policy visible at
 * every non-default producer callsite and packs it into the shared event's
 * flags for runtime validation without changing the TCG helper ABI.  The
 * class wrapper below is the common exact-width path. */
static void gen_sem_mem_access_f01_raw_class_policy(
    TCGv t_addr, target_ulong size, target_ulong pc, bool is_store,
    SemOpClass cls, SemIntervalPolicy policy, SemProducerId producer) {
    if (!sem_events_active()) return;
    TCGv t_size = tcg_const_tl(size);
    TCGv t_pc = tcg_const_tl(pc);
    TCGv_i32 t_flags = tcg_const_i32(SEM_MEM_FLAGS_WITH_POLICY(
        (is_store ? 1 : 0) | 8, policy, producer));
    TCGv_i32 t_cls = tcg_const_i32(cls);
    gen_helper_sem_mem_access(cpu_env, t_addr, t_size, t_pc, t_flags,
                              t_cls);
    tcg_temp_free(t_size);
...
static void gen_sem_mem_access_f01_class_policy(
    TCGv t_addr, TCGMemOp ot, target_ulong pc, bool is_store,
    SemOpClass cls, SemIntervalPolicy policy, SemProducerId producer) {
    gen_sem_mem_access_f01_raw_class_policy(
        t_addr, 1 << (ot & MO_SIZE), pc, is_store, cls, policy, producer);
}

static void gen_sem_mem_access_f01_class_policy_default(
    TCGv t_addr, TCGMemOp ot, target_ulong pc, bool is_store,
    SemOpClass cls, SemIntervalPolicy policy) {
    gen_sem_mem_access_f01_class_policy(
        t_addr, ot, pc, is_store, cls, policy, sem_default_producer(cls));
}

static void gen_sem_descriptor_access_f01(TCGv addr, target_ulong size,
                                          target_ulong pc, bool is_store) {
    gen_sem_mem_access_f01_raw_class_policy(
        addr, size, pc, is_store, SEM_OP_INTEGER, SEM_INTERVAL_MULTIPART,
        SEM_PRODUCER_INTEGER_DESCRIPTOR);
}

static void gen_sem_mpx_bndmov_access(TCGv addr, target_ulong size,
                                      target_ulong pc, bool is_store) {
    gen_sem_mem_access_f01_raw_class_policy(
        addr, size, pc, is_store, SEM_OP_MPX, SEM_INTERVAL_PAIRED,
...
static void gen_sem_mem_access_f01_class(TCGv t_addr, TCGMemOp ot,
                                         target_ulong pc, bool is_store,
                                         SemOpClass cls) {
    gen_sem_mem_access_f01_class_policy_default(
        t_addr, ot, pc, is_store, cls, SEM_INTERVAL_EXACT_WIDTH);
}

static void gen_sem_mem_xchg_f01(TCGv addr, TCGMemOp ot, target_ulong pc)
{
    gen_sem_mem_access_f01_class_policy(
        addr, ot, pc, true, SEM_OP_ATOMIC_RMW,
        SEM_INTERVAL_EXACT_WIDTH, SEM_PRODUCER_ATOMIC_XCHG);
}
 ...
 * the live frame stack (OSPREY-only consumer; provenance no-op). */
static void gen_sem_rsp_update(TCGv new_sp, target_ulong pc) {
    if (!sem_events_active()) return;
    TCGv t_pc = tcg_const_tl(pc);
    gen_helper_sem_rsp_update(cpu_env, new_sp, t_pc);
    tcg_temp_free(t_pc);
}

static void gen_sem_call(TCGv callee_pc) {
    if (!sem_events_active()) return;
    gen_helper_sem_call(cpu_env, callee_pc, cpu_regs[R_ESP]);
}

static void gen_sem_ret(TCGv pc) {
    if (!sem_events_active()) return;
    gen_helper_sem_ret(cpu_env, pc, cpu_regs[R_ESP]);
}
...
struct DisasContext {
    ...
    /* Sem layer: suppress the reg write-back invalidation for one
     * instruction so the addsub helper (emitted after gen_op) can read
     * the pre-op tag and fold the post-op delta. */
    bool prov_skip_invalidate;
    /* Sem layer: RET updates RSP before the explicit return hook.
     * Suppress the generic RSP-resynchronization helper for that one
     * write so the return hook owns the activation pop exactly once
     * (including ret imm). */
    bool osprey_skip_rsp_update;
    /* Manifest family for generic memory events in this instruction.  An
     * INVALID value selects the class-specific default; stack/string/I/O
     * translators override it so their narrower width rows are enforced. */
    SemProducerId sem_producer;
...
static void gen_sem_set_ea_mode(DisasContext *s) {
    if (!sem_events_active()) return;
    uint32_t mode = (uint32_t)s->aflag |
                    (((uint32_t)s->override + 1) << 8);
    TCGv_i32 t_mode = tcg_const_i32(mode);
    gen_helper_sem_set_ea_mode(cpu_env, t_mode);
    tcg_temp_free_i32(t_mode);
}

/* Effective-address metadata: emitted before a guest access, consumed
 * after it succeeds (mode carries aflags/override; the OSPREY F01 gate
 * is consumer-side policy). */
static void gen_sem_set_ea(DisasContext *s, int base_reg, int index_reg,
                           int scale, target_long disp) {
    if (!sem_events_active()) return;
    TCGv_i32 t_base = tcg_const_i32(base_reg);
    TCGv_i32 t_index = tcg_const_i32(index_reg);
    TCGv_i32 t_scale = tcg_const_i32(scale);
    TCGv t_disp = tcg_const_tl((target_ulong)disp);
    /* Pack address-width and segment-override state into one argument:
     * low 8 bits = TCGMemOp aflag, next bits = override + 1 (zero means
     * no override). */
    uint32_t mode = (uint32_t)s->aflag |
                    (((uint32_t)s->override + 1) << 8);
    TCGv_i32 t_mode = tcg_const_i32(mode);
    gen_helper_sem_set_ea(cpu_env, t_base, t_index, t_scale, t_disp,
                          t_mode);
    tcg_temp_free_i32(t_base);
    ...
    tcg_temp_free(t_disp);
    tcg_temp_free_i32(t_mode);
    /* Explicit base/index concrete values for the OSPREY EA record
     * (env->regs may be stale inside helpers). */
    TCGv t_base_val = tcg_const_tl(0);
    TCGv t_index_val = tcg_const_tl(0);
    if (base_reg >= 0 && base_reg < CPU_NB_REGS) {
        tcg_gen_mov_tl(t_base_val, cpu_regs[base_reg]);
    }
    if (index_reg >= 0 && index_reg < CPU_NB_REGS) {
        tcg_gen_mov_tl(t_index_val, cpu_regs[index_reg]);
    }
 ...
 * post-success interval and carry the operation class and manifest family;
 * paired helpers use raw event functions so constituent stores do not create
 * F01 rows. */
static SemProducerId sem_instruction_producer(const DisasContext *s,
                                              SemOpClass cls)
{
    return s->sem_producer == SEM_PRODUCER_INVALID
        ? sem_default_producer(cls) : s->sem_producer;
}
...
static SemProducerId sem_qemu_producer(SemOpClass cls)
{
    return cls == SEM_OP_SIMD ? SEM_PRODUCER_SIMD_SPECIAL
...
static void gen_sem_qemu_ld_tl(DisasContext *s, TCGv dst, TCGv addr,
    ...
    gen_sem_mem_access_f01_raw_class_policy(
        addr, 1 << (ot & MO_SIZE), s->pc_start, false, cls,
        SEM_INTERVAL_EXACT_WIDTH, sem_qemu_producer(cls));
}
...
static void gen_sem_qemu_st_tl(DisasContext *s, TCGv src, TCGv addr,
    ...
    gen_sem_on_store_class(-1, addr, ot, cls, s->pc_start);
    gen_sem_mem_access_f01_raw_class_policy(
        addr, 1 << (ot & MO_SIZE), s->pc_start, true, cls,
        SEM_INTERVAL_EXACT_WIDTH, sem_qemu_producer(cls));
}
...
static void gen_op_mov_reg_v(DisasContext *s, TCGMemOp ot, int reg, TCGv t0)
{
    ...
    /* Default: invalidate the destination register's provenance tag and
     * OSPREY origin, unless suppressed (reg-reg ADD/SUB propagation,
     * lea).  For high-byte writes (AH/CH/DH/BH in 32-bit mode) the
     * architectural full register is reg - 4, not reg: byte_reg_is_xH
     * writes cpu_regs[reg - 4], so the shadow slot to invalidate is
     * reg - 4 as well. */
    if (!s->prov_skip_invalidate) {
        ...
        gen_sem_reg_invalidate(full_reg, s->pc_start);
    }
    ...
    if (reg == R_ESP && ot == MO_64 && !s->osprey_skip_rsp_update) {
        gen_sem_rsp_update(cpu_regs[R_ESP], s->pc_start);
    }
}
...
static inline void gen_op_ld_v_moffs(DisasContext *s, int idx, TCGv t0,
                                     TCGv a0)
{
    s->sem_producer = SEM_PRODUCER_INTEGER_MOFFS;
    gen_op_ld_v(s, idx, t0, a0);
}
                                         ...
                                         TCGv a0, SemOpClass cls)
{
     ...
     * stores, string stores) emit gen_sem_on_store AFTER this call to
    ...
    gen_sem_on_store_class(OR_TMP0, a0, idx, cls, s->pc_start);
}
...
static inline void gen_op_st_v_moffs(DisasContext *s, int idx, TCGv t0,
                                     TCGv a0)
{
    s->sem_producer = SEM_PRODUCER_INTEGER_MOFFS;
    gen_op_st_v(s, idx, t0, a0);
}
...
{
    s->sem_producer = SEM_PRODUCER_INTEGER_STRING;
    gen_string_movl_A0_ESI(s);
    gen_sem_set_ea(s, R_ESI, -1, 0, 0);
    gen_op_ld_v(s, ot, s->T0, s->A0);
    gen_sem_mem_access(s->A0, ot, s->pc_start, false);
    gen_string_movl_A0_EDI(s);
    gen_sem_set_ea(s, R_EDI, -1, 0, 0);
    gen_op_st_v(s, ot, s->T0, s->A0);
    /* The store event was emitted by gen_op_st_v (OR_TMP0 source → both
     * consumers invalidate the slot).  The F01 store access follows. */
...
static inline void gen_stos(DisasContext *s, TCGMemOp ot)
{
    ...
    gen_sem_set_ea(s, R_EDI, -1, 0, 0);
    gen_op_st_v(s, ot, s->T0, s->A0);
    /* The store event was emitted by gen_op_st_v (OR_TMP0 → both
     * consumers invalidate the slot).  The F01 store access follows. */
    gen_sem_mem_access(s->A0, ot, s->pc_start, true);
    gen_op_movl_T0_Dshift(s, ot);
    gen_op_add_reg_T0(s, s->aflag, R_EDI);
...
static inline void gen_lods(DisasContext *s, TCGMemOp ot)
{
     ...
     * OSPREY: reload the origin from the stack shadow. */
    gen_sem_on_load(R_EAX, s->A0, ot, s->pc_start);
    gen_sem_mem_access(s->A0, ot, s->pc_start, false);
    gen_op_movl_T0_Dshift(s, ot);
    gen_op_add_reg_T0(s, s->aflag, R_ESI);
}
...
{
    s->sem_producer = SEM_PRODUCER_INTEGER_INS_OUTS;
    ...
    /* The restartability write is an implementation pre-store, not the
     * completed INS result.  Invalidate its shadow but publish exactly one
     * F01 event only after the port read and architectural result store. */
    s->sem_skip_f01_store = sem_events_active();
    gen_op_st_v(s, ot, s->T0, s->A0);
    ...
    if (mod == 3) {
        if (is_store) {
            ...
            if (reg != OR_TMP0 && ot == MO_64) {
                gen_sem_reg_copy(rm, reg, s->pc_start);
            }
        } else {
        ...
        TCGv ea = gen_lea_modrm_1(s, a);
        gen_lea_v_seg(s, s->aflag, ea, a.def_seg, s->override);
        /* Store the architectural EA before every supported memory op.
         * The runtime helper turns truncating/segment forms into UNKNOWN,
         * preserving exact-bounds fallback in symbolic mode. */
        gen_sem_set_ea(s, a.base, a.index, a.scale, a.disp);
        if (is_store) {
            if (reg != OR_TMP0)
                gen_op_mov_v_reg(s, ot, s->T0, reg);
            ...
            gen_sem_on_store_class(reg, s->A0, ot, cls, s->pc_start);
...
{
    s->sem_producer = SEM_PRODUCER_INTEGER_STACK;
...
{
    s->sem_producer = SEM_PRODUCER_INTEGER_ENTER;
    TCGMemOp d_ot = mo_pushpop(s, s->dflag);
    ...
    if (sem_active) {
        ...
        gen_sem_set_ea_mode(s);
        gen_sem_mem_access_f01_raw_class_policy(
            s->A0, size, s->pc_start, true, SEM_OP_INTEGER,
            SEM_INTERVAL_MULTIPART, SEM_PRODUCER_INTEGER_ENTER);
        if (level != 0) {
            for (int i = 1; i < level; ++i) {
                ...
                gen_sem_set_ea_mode(s);
                gen_sem_mem_access_f01_raw_class_policy(
                    s->A0, size, s->pc_start, false, SEM_OP_INTEGER,
                    SEM_INTERVAL_MULTIPART, SEM_PRODUCER_INTEGER_ENTER);
...
static inline void gen_stq_env_A0(DisasContext *s, int offset)
{
    ...
    /* SSE scalar store: not a modeled pointer spill — invalidate the
     * shadow slot, then record the F01 access fact (after the store so
     * a fault records nothing). */
    gen_sem_set_ea_mode(s);
    gen_sem_on_store_class(OR_TMP0, s->A0, MO_64, SEM_OP_SIMD, s->pc_start);
...
static inline void gen_ldo_env_A0(DisasContext *s, int offset)
{
    ...
    gen_sem_mem_access_f01_raw_class_policy(
        s->A0, 16, s->pc_start, false, SEM_OP_SIMD,
        SEM_INTERVAL_PAIRED, SEM_PRODUCER_SIMD_VECTOR);
}
...
static inline void gen_sto_env_A0(DisasContext *s, int offset)
{
    ...
    /* Invalidate after each constituent store so a fault on a later
     * store cannot leave stale metadata from an earlier one.  The 128-bit
    ...
    gen_sem_on_store_class(OR_TMP0, s->tmp0, MO_64, SEM_OP_SIMD, s->pc_start);
    gen_sem_mem_access_f01_raw_class_policy(
        s->A0, 16, s->pc_start, true, SEM_OP_SIMD,
        SEM_INTERVAL_PAIRED, SEM_PRODUCER_SIMD_VECTOR);
}
    ...
    if (sse_fn_epp == SSE_SPECIAL) {
        ...
        do_0f_38_fx:
            ...
            case 0x3f6: /* mulx By, Gy, rdx, Ey */
                ...
                gen_sem_reg_invalidate(s->vex_v, s->pc_start);
                gen_sem_reg_invalidate(reg, s->pc_start);
                break;
        ...
        case 0x13a:
            ...
            if (sse_fn_eppi == SSE_SPECIAL) {
                ...
                case 0x16:
                    ...
                    if (mod == 3) {
                        gen_sem_reg_invalidate(rm, s->pc_start);
                    }
                    break;
                case 0x17: /* extractps */
                    tcg_gen_ld32u_tl(s->T0, cpu_env, offsetof(CPUX86State,
...
static target_ulong disas_insn(DisasContext *s, CPUState *cpu)
{
 ...
 next_byte:
    ...
    s->aflag = aflag;
    s->dflag = dflag;
    s->sem_producer = SEM_PRODUCER_INVALID;
 ...
 reswitch:
    ...
    case 0x38 ... 0x3d:
        {
            ...
            case 0: /* OP Ev, Gv */
                ...
                if (mod != 3) {
                    ...
                    TCGv ea = gen_lea_modrm_1(s, a);
                    gen_lea_v_seg(s, s->aflag, ea, a.def_seg, s->override);
                    /* Store every architectural EA; unsupported modes
                ...
                if (sem_events_active() && opreg != OR_TMP0 &&
                ...
                if (sem_events_active() && opreg != OR_TMP0 &&
                    (op == OP_ADDL || op == OP_SUBL)) {
                    gen_sem_reg_addsub_reg(opreg, reg, op == OP_SUBL,
                                           s->pc_start);
                }
                break;
            case 1: /* OP Gv, Ev */
                ...
                if (sem_events_active() && mod == 3 &&
                    (op == OP_ADDL || op == OP_SUBL)) {
                    s->prov_skip_invalidate = true;
                }
                gen_op(s, op, ot, reg);
                /* reg-reg ADD/SUB provenance propagation (after write-back).
                 * Only for register Ev sources (mod == 3): a memory source
                 * value carries no register provenance to fold. */
                if (sem_events_active() && mod == 3 &&
                    (op == OP_ADDL || op == OP_SUBL)) {
                    gen_sem_reg_addsub_reg(reg, rm, op == OP_SUBL,
                                           s->pc_start);
                }
                break;
            case 2: /* OP A, Iv */
                ...
                if (sem_events_active() && ot == MO_64 &&
                    (op == OP_ADDL || op == OP_SUBL)) {
                    int64_t imm = (int64_t)(int32_t)(uint32_t)val;
                    int64_t delta = (op == OP_SUBL) ? -imm : imm;
                    gen_sem_reg_addsub_imm(OR_EAX, delta,
                                           s->prov_pre_snapshot,
                                           s->pc_start);
    ...
    case 0x83:
        {
            ...
            if (mod != 3) {
                ...
                TCGv ea = gen_lea_modrm_1(s, a);
                gen_lea_v_seg(s, s->aflag, ea, a.def_seg, s->override);
                /* Store every architectural EA; unsupported modes become
             ...
             * Suppress the write-back invalidation so the post-op helper
             * can fold the pre-op tag; snapshot the pre-op register value
             * for the helper's consistency check. */
            if (sem_events_active() && mod == 3 && ot == MO_64 &&
                (op == OP_ADDL || op == OP_SUBL)) {
                s->prov_skip_invalidate = true;
                s->prov_pre_snapshot = tcg_temp_new();
            ...
            if (sem_events_active() && mod == 3 && ot == MO_64 &&
                ...
                int64_t imm = (int64_t)(int32_t)(uint32_t)val;
                int64_t delta = (op == OP_SUBL) ? -imm : imm;
                gen_sem_reg_addsub_imm(opreg, delta, s->prov_pre_snapshot,
                                       s->pc_start);
                s->prov_pre_snapshot = NULL;
            }
        }
    ...
    case 0xff: /* GRP5 */
        ...
        case 2: /* call Ev */
            ...
            gen_push_v(s, s->T1);
            /* Shared call boundary: fires after the return-address push,
             * so the event carries the precise callee-entry RSP. */
            if (sem_events_active()) {
                TCGv t_callee = tcg_temp_new();
                tcg_gen_mov_tl(t_callee, s->T0);
                gen_sem_call(t_callee);
        ...
        case 1: /* CMPXCHG8, CMPXCHG16 */
            ...
            if (dflag == MO_64) {
                ...
                gen_sem_on_store_class(OR_TMP0, s->A0, MO_64,
                                       SEM_OP_PAIRED, s->pc_start);
                tcg_gen_addi_tl(s->tmp0, s->A0, 8);
                gen_sem_on_store_class(OR_TMP0, s->tmp0, MO_64,
                                       SEM_OP_PAIRED, s->pc_start);
                gen_sem_mem_access_f01_raw_class_policy(
                    s->A0, 16, s->pc_start, true, SEM_OP_PAIRED,
                    SEM_INTERVAL_PAIRED, SEM_PRODUCER_PAIRED_CMPXCHG16B);
                gen_sem_reg_invalidate(R_EAX, s->pc_start);
                gen_sem_reg_invalidate(R_EDX, s->pc_start);
                set_cc_op(s, CC_OP_EFLAGS);
                break;
            ...
            gen_sem_on_store_class(OR_TMP0, s->A0, MO_64,
                                   SEM_OP_PAIRED, s->pc_start);
            gen_sem_mem_access_f01_raw_class_policy(
                s->A0, 8, s->pc_start, true, SEM_OP_PAIRED,
                SEM_INTERVAL_PAIRED, SEM_PRODUCER_PAIRED_CMPXCHG8B);
            gen_sem_reg_invalidate(R_EAX, s->pc_start);
            gen_sem_reg_invalidate(R_EDX, s->pc_start);
            set_cc_op(s, CC_OP_EFLAGS);
            break;
    ...
    case 0x50 ... 0x57: /* push */
        ...
        gen_push_v(s, s->T0);
        /* Push spills the register origin/tag to the stack shadow.  The
         * actual store width is mo_pushpop(s, s->dflag) (MO_64 in 64-bit
         * mode); passing s->dflag (MO_32) would make the helper see a
         * non-full-width store and invalidate the slot instead. */
        gen_sem_on_store((b & 7) | REX_B(s), s->A0,
                         mo_pushpop(s, s->dflag), s->pc_start);
        break;
    case 0x58 ... 0x5f: /* pop */
        ot = gen_pop_T0(s);
        ...
        /* Pop restores the tag/origin from the stack shadow. */
        gen_sem_on_load((b & 7) | REX_B(s), s->A0, ot, s->pc_start);
        break;
    case 0x60: /* pusha */
        if (CODE64(s))
            goto illegal_op;
    ...
    case 0xc7: /* mov Ev, Iv */
        ...
        if (mod != 3) {
            gen_op_st_v(s, ot, s->T0, s->A0);
            /* Immediate store: the gen_op_st_v store event (OR_TMP0
             * source) invalidates the slot for both consumers; the F01
             * store access fact follows. */
            gen_sem_mem_access(s->A0, ot, s->pc_start, true);
        } else {
    ...
    case 0x8b: /* mov Ev, Gv */
        ...
        gen_ldst_modrm(env, s, modrm, ot, OR_TMP0, 0);
        gen_op_mov_reg_v(s, ot, reg, s->T0);
        /* Restore tag/origin from memory shadow (spill reload). */
        if ((modrm >> 6) != 3) {
    ...
    case 0x1bf: /* movswS Gv, Eb */
        {
            ...
            if (mod == 3) {
                ...
                gen_op_mov_reg_v(s, d_ot, reg, s->T0);
                gen_sem_on_load(reg, s->A0, s_ot, s->pc_start);
                gen_sem_mem_access(s->A0, s_ot, s->pc_start, false);
            }
        }
        break;
    ...
    case 0x8d: /* lea */
        ...
        reg = ((modrm >> 3) & 7) | rex_r;
        {
            ...
            /* Provenance: lea dst, [base + disp] propagates the base
             * identity (checked constant offset).  Indexed forms
             * [base + index*scale + disp] propagate the base identity
             * with the verified runtime delta (index*scale + disp)
             * folded into the offset — same checked arithmetic as
             * provenance_lea_imm, delta computed at runtime.  The
             * write-back invalidation is suppressed so the helper can
             * read the pre-op dst tag; the base's pre-instruction value
             * is snapshotted when dst == base. */
            TCGv pre_base = NULL;
            if (sem_events_active() && dflag == MO_64 && a.base >= 0) {
                /* Constant-offset LEA is accepted by both consumers.
                 * Indexed LEA needs the pre-write base for provenance;
                 * OSPREY receives the event but conservatively kills the
                 * destination until Stage 2.3 defines its policy. */
    ...
    case 0xa3:
        {
            ...
            gen_add_A0_ds_seg(s);
            if ((b & 2) == 0) {
                gen_op_ld_v_moffs(s, ot, s->T0, s->A0);
                gen_op_mov_reg_v(s, ot, R_EAX, s->T0);
            } else {
                gen_op_mov_v_reg(s, ot, s->T0, R_EAX);
                gen_op_st_v_moffs(s, ot, s->T0, s->A0);
            }
        }
        break;
    case 0xd7: /* xlat */
    ...
    case 0x87: /* xchg Ev, Gv */
        ...
        do_xchg_reg:
            ...
            gen_sem_on_store_class(OR_TMP0, s->A0, ot,
                                   SEM_OP_ATOMIC_RMW, s->pc_start);
            gen_sem_mem_xchg_f01(s->A0, ot, s->pc_start);
        }
        break;
    case 0xc4: /* les Gv */
    ...
    case 0xd8 ... 0xdf:
        ...
        if (mod != 3) {
            ...
            case 0x38 ... 0x3b: /* filds, fisttps, fists, fistps */
                ...
                case 1:
                    ...
                    case 1:
                        ...
                        gen_sem_set_ea_mode(s);
                        gen_sem_on_store_x87(OR_TMP0, s->A0, MO_32, s->pc_start);
                    ...
                    case 2:
                        ...
                        /* FPU store: invalidate + F01 after the store. */
                        gen_sem_set_ea_mode(s);
                        gen_sem_on_store_x87(OR_TMP0, s->A0, MO_64, s->pc_start);
                    ...
                    default:
                        ...
                        /* FPU store: invalidate + F01 after the store. */
                        gen_sem_set_ea_mode(s);
                        gen_sem_on_store_x87(OR_TMP0, s->A0, MO_16, s->pc_start);
            ...
            case 0x0c: /* fldenv mem */
                ...
                if (!(CODE64(s) && dflag == MO_16)) {
                    gen_sem_mem_access_f01_raw_class_policy(
                        s->A0, dflag == MO_16 ? 14 : 28, s->pc_start, false,
                        SEM_OP_X87_HELPER, SEM_INTERVAL_RAW,
            ...
            case 0x0e: /* fnstenv m14/28 */
                ...
                gen_sem_on_store_x87(OR_TMP0, s->A0, MO_64, s->pc_start);
                tcg_gen_addi_tl(s->tmp0, s->A0, 8);
                gen_sem_on_store_x87(OR_TMP0, s->tmp0, MO_64, s->pc_start);
                if (dflag != MO_16) {
                    tcg_gen_addi_tl(s->tmp0, s->A0, 16);
                    gen_sem_on_store_x87(OR_TMP0, s->tmp0, MO_64, s->pc_start);
                    tcg_gen_addi_tl(s->tmp0, s->A0, 24);
                    gen_sem_on_store_x87(OR_TMP0, s->tmp0, MO_32, s->pc_start);
                    gen_sem_mem_access_f01_raw_class_policy(
                        s->A0, 28, s->pc_start, true, SEM_OP_X87_HELPER,
                        SEM_INTERVAL_RAW, SEM_PRODUCER_X87_ENVIRONMENT);
                }
                break;
            ...
            case 0x1d: /* fldt mem */
                ...
                gen_sem_mem_access_f01_raw_class_policy(
                    s->A0, 10, s->pc_start, false, SEM_OP_X87_HELPER,
                    SEM_INTERVAL_RAW, SEM_PRODUCER_X87_RAW);
                break;
            case 0x1f: /* fstpt m80 */
                ...
                gen_sem_on_store_x87(OR_TMP0, s->A0, MO_64, s->pc_start);
                tcg_gen_addi_tl(s->tmp0, s->A0, 8);
                gen_sem_on_store_x87(OR_TMP0, s->tmp0, MO_16, s->pc_start);
                gen_sem_mem_access_f01_raw_class_policy(
                    s->A0, 10, s->pc_start, true, SEM_OP_X87_HELPER,
                    SEM_INTERVAL_RAW, SEM_PRODUCER_X87_RAW);
                gen_helper_fpop(cpu_env);
                break;
            case 0x2c: /* frstor mem */
                ...
                if (!(CODE64(s) && dflag == MO_16)) {
                    gen_sem_mem_access_f01_raw_class_policy(
                        s->A0, dflag == MO_16 ? 94 : 108, s->pc_start, false,
                        SEM_OP_X87_HELPER, SEM_INTERVAL_RAW,
            ...
            case 0x2e: /* fnsave m94/108 */
                ...
                if (dflag != MO_16) {
                    ...
                    gen_sem_on_store_x87(OR_TMP0, s->tmp0, MO_32, s->pc_start);
                    gen_sem_mem_access_f01_raw_class_policy(
                        s->tmp4, 108, s->pc_start, true, SEM_OP_X87_HELPER,
                        SEM_INTERVAL_RAW, SEM_PRODUCER_X87_SAVED_STATE);
                } else if (!CODE64(s)) {
                    gen_sem_mem_access_f01_raw_class_policy(
                        s->tmp4, 94, s->pc_start, true, SEM_OP_X87_HELPER,
                        SEM_INTERVAL_RAW, SEM_PRODUCER_X87_SAVED_STATE);
                }
                break;
            case 0x2f: /* fnstsw m */
    ...
    case 0xc2: /* ret im */
        ...
        /* Shared return boundary carries the post-pop RSP. */
        gen_sem_ret(s->T0);
    ...
    case 0xe8: /* call im */
        {
            ...
            gen_push_v(s, s->T0);
            /* Shared direct-call boundary after the return-address push. */
            if (sem_events_active()) {
...
#endif
#endif
        /* E9-relocated call: the stub already pushed the return address,
         * so the shared boundary sees the precise callee-entry RSP. */
        if (sem_events_active() &&
    ...
    case 0x62: /* bound */
        ...
        if (ot == MO_16) {
            /* helper_boundw publishes one exact 4-byte F01 interval only
             * after both bound loads and the architectural check succeed. */
            gen_helper_boundw(cpu_env, s->A0, s->tmp2_i32, t_bound_pc);
        } else {
            /* helper_boundl publishes one exact 8-byte F01 interval only
             * after both bound loads and the architectural check succeed. */
    ...
    case 0x1a2: /* cpuid */
        ...
        gen_sem_reg_invalidate(R_EAX, s->pc_start);
        gen_sem_reg_invalidate(R_EBX, s->pc_start);
        gen_sem_reg_invalidate(R_ECX, s->pc_start);
        gen_sem_reg_invalidate(R_EDX, s->pc_start);
        break;
    case 0xf4: /* hlt */
        if (s->cpl != 0) {
    ...
    case 0x101:
        ...
        CASE_MODRM_MEM_OP(0): /* sgdt */
            ...
            gen_op_st_v_raw_class(s, CODE64(s) + MO_32, s->T0, s->A0,
                                  SEM_OP_INTEGER);
            gen_sem_descriptor_access_f01(s->tmp4, CODE64(s) ? 10 : 6,
                                           s->pc_start, true);
            break;
        ...
        if (s->flags & HF_MPX_EN_MASK) {
            ...
            if (prefixes & PREFIX_REPZ) {
                ...
                if (mod == 3) {
                    ...
                    gen_sem_mpx_bndmov_access(s->tmp4, CODE64(s) ? 16 : 8,
                                               s->pc_start, false);
            ...
            if (mod != 3 && (prefixes & PREFIX_REPZ)) {
                ...
                if (mod == 3) {
                    ...
                    if (CODE64(s)) {
                        ...
                        gen_sem_mpx_bndmov_access(s->tmp4, 16, s->pc_start,
                                                   true);
                    } else {
                        tcg_gen_qemu_st_i64(cpu_bndl[reg], s->A0,
                                            s->mem_index, MO_LEUL);
                        ...
                        gen_sem_mpx_bndmov_access(s->tmp4, 8, s->pc_start,
                                                   true);
                    }
                }
            } else if (mod != 3) {
    ...
    case 0x1ae:
        ...
        CASE_MODRM_MEM_OP(2): /* ldmxcsr */
            ...
            gen_sem_mem_access_f01_raw_class_policy(
                s->A0, 4, s->pc_start, false, SEM_OP_SIMD,
